    ...
    def toLineModifiers(self, maxLines=100) -> list[LineModifier]:
        ...
        for modifier in self:
            if modifier.operation == 'discard':
                ...
                ret[modifier.start].syllableLengths = list(map(lambda x: pyass.timedelta(centiseconds=int(x)), modifier.rest[2:]))
...
def modify_song(song: Song, modifiers: list[Modifier]) -> Song:
    lineModifiers = Modifiers(modifiers).toLineModifiers(maxLines=len(song.lyrics))
    outLyrics = []
    for line, modifier in zip(song.lyrics, lineModifiers):
        ...
        if modifier.shouldRunKaraTemplater:
            song.shouldRunKaraokeTemplater = True

        if modifier.shouldOverwriteTitle:
            song.title.romaji = modifier.titleRomaji
            song.title.en = modifier.titleEN

        if modifier.shouldOverwriteArtist:
            song.creators.artist = modifier.artist
    ...
    if lineModifiers[0].newOrder is None:
        song.lyrics = outLyrics
        return song

    song.lyrics = []
    for lineNum in lineModifiers[0].newOrder:
        song.lyrics.append(outLyrics[lineNum])
    return song
